...
const initialState= {
  questions: [],
  status: "loading",   // status could be loading, error, ready, active , finished
  index: 0,
  answer: null,
  points: 0,
  highscore:0,
...
function reducer(state,action){
  switch(action.type){
    ...
    case "start":
      return {
        ...state , status : "active", secoundsRemaining: state.questions.length * SECS_PER_QUESTION
      }
    case "newAnswer":
    ...
    case "nextQuestion":
      ...
      case "tick":
        return {
          ...state, secoundsRemaining: state.secoundsRemaining - 1, status: state.secoundsRemaining === 0 ? "finished" : state.status
    }
    default:{
      throw new Error("Action Known")
    }
  }
}
...
export default function App(){
  ...
  useEffect(function(){
    ...
    fetch("https://raw.githubusercontent.com/mfa1zan/react-quiz/main/data/questions.json").then(res=>res.json()).then(data=>dispatch({type:"dataRecieved", payload: data.questions})).catch(err=>dispatch({type:"dataFailed"}))
